...
//  ExchangeRates
//
//  Created by Matheus Franceschini on 17/10/23.
//
...
extension RateFluctuationDetailView {
    @MainActor class ViewModel: ObservableObject {
        ...
        func doComparison(with rateFluctuation: RateFluctuationModel) {
        ...
        private func doFetchRatesFluctuation() {
            if let baseCurrency {
                ...
                fluctuationDataProvider?.fetchFluctuation(by: baseCurrency, from: [], startDate: startDate.toString(), endDate: endDate)
                    .sink(receiveCompletion: { completion in
                        switch completion {
                        case .finished: self.currentState = .success
                        case .failure(_): print("Falha")
                        }
